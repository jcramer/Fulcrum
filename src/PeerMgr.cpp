...
void PeerMgr::startup()
{
    ...
    if (const auto net = BTC::NetFromName(chain); !QVector<BTC::Net>{{BTC::Net::TestNet, BTC::Net::TestNet4, BTC::Net::ScaleNet, BTC::Net::NexTestNet, BTC::Net::MainNet}}.contains(net))
    ...
    else if (net == BTC::Net::NexTestNet)
        parseServersDotJson(pathPrefix + "servers_nextest.json");  // BCH only -- will implicitly throw if somehow the coin is BTC (should never happen)
    else
